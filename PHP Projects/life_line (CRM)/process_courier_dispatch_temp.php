...
for ( $i = 0; $i < $total_number; $i++ ) {
  executeQuery( "UPDATE `order_dispatch_info` SET order_amount='$order_amount[$i]', package_weight='$package_weight[$i]',customer_feedback='$customer_feedback[$i]',dispatch_id='$order_courier_dispatch_id' WHERE order_id='$order_id[$i]'" );
...
//                echo $courier_cost[$i];
            ...
            $dr_tid=executeQuery("INSERT INTO `m_account_detail` (`sno`, `m_accountid`, `tr_date`, `amount`, `type`, `tr_description`, `invno`, `entered_by`, `edited_at`, `edited_by`, `info`, `timestamp`, `user_verified`, `admin_verified`) VALUES (NULL, '$company_dispatch_ledger', '$cheque_date', '$courier_cost[$i]', '$order_id[$i]', 'Automated Dispatch# $order_courier_dispatch_id$order_courier_dispatch_id Payment Order ID $order_id[$i] Receiver $receiver_name Dated $order_date worth $order_amount[$i] ', '$voucher_id', '$_SESSION[email]', '$currentDateTime', NULL, 'Dr', '$currentDateTime', NULL, NULL)"); 
    
     //    company ko Payment di
    executeQuery("INSERT INTO `m_account_detail` (`sno`, `m_accountid`, `tr_date`, `amount`, `type`, `tr_description`, `invno`, `entered_by`, `edited_at`, `edited_by`, `info`, `timestamp`, `user_verified`, `admin_verified`) VALUES (NULL, '$default_cash_handler_ledger', '$cheque_date', '$courier_cost[$i]', '$order_id[$i]', 'Automated Dispatch# $order_courier_dispatch_id$order_courier_dispatch_id Payment Order ID $order_id[$i] Receiver $receiver_name Dated $order_date worth $order_amount[$i]', '$voucher_id', '$_SESSION[email]', '$currentDateTime', NULL, 'Cr', '$currentDateTime', NULL, NULL)");
            }
...
//                echo $courier_cost[$i];
    ...
    //Cash Account ko mila
            ...
            $dr_tid=executeQuery("INSERT INTO `m_account_detail` (`sno`, `m_accountid`, `tr_date`, `amount`, `type`, `tr_description`, `invno`, `entered_by`, `edited_at`, `edited_by`, `info`, `timestamp`, `user_verified`, `admin_verified`) VALUES (NULL, '$company_receiving_ledger', '$cheque_date', 'customer_feedback[$i]', '$order_id[$i]', 'Automated Dispatch# $order_courier_dispatch_id Payment Order ID $order_id[$i] Receiver $receiver_name Dated $order_date worth $order_amount[$i] ', '$voucher_id', '$_SESSION[email]', '$currentDateTime', NULL, 'Cr', '$currentDateTime', NULL, NULL)"); 
    
                //    company ko Se Nikla
    executeQuery("INSERT INTO `m_account_detail` (`sno`, `m_accountid`, `tr_date`, `amount`, `type`, `tr_description`, `invno`, `entered_by`, `edited_at`, `edited_by`, `info`, `timestamp`, `user_verified`, `admin_verified`) VALUES (NULL, '$default_cash_handler_ledger', '$cheque_date', 'customer_feedback[$i]', '$order_id[$i]', 'Automated Dispatch# $order_courier_dispatch_id Payment Order ID $order_id[$i] Receiver $receiver_name Dated $order_date worth $order_amount[$i]', '$voucher_id', '$_SESSION[email]', '$currentDateTime', NULL, 'Dr', '$currentDateTime', NULL, NULL)");
            }
...
executeQuery( "UPDATE `m_account_detail` SET status='Return' WHERE type!='' AND tr_description LIKE '%Return Stock Received%';" );
