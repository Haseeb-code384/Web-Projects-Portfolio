...
$other_charges=$other_charges/$total_number;
    ...
    $cash_receiving_ledger=$couries_account_details['cash_receiving_ledger'];
       ...
       $voucher_id=showQuery("SELECT payment_accounts_voucher_receiving FROM `order_dispatch_info` WHERE order_id='$order_id[0]'");
executeQuery("UPDATE `daily_voucher`  SET date='$cheque_date' WHERE id='$voucher_id'");
if($voucher_id=="")
{
...
executeQuery("DELETE FROM `m_account_detail` WHERE invno='$voucher_id'");
for($i=0;$i<$total_number;$i++)
{
  ...
  $tracking_id = showQuery( "SELECT tracking_id FROM `order_dispatch_info` WHERE order_id='$order_id[$i]'" );
    ...
    executeQuery("INSERT INTO `inquiry_status_history` (`id`, `inquiry_id`, `type`, `status`, `time`, `allocated_to`, `allocated_by`, `order_no`, `comments`, `duration`, `call_type`, `entered_by`, `call_status`, `medicine_status`, `patient_feedback`) VALUES (NULL, '$inquiy_id', 'Order Handling', '$d_status[$i]', '$currentDateTime', '$_SESSION[email]', '$seller', '$order_id[$i]', 'Edit $d_status[$i] Bill# $payment_receiving_id', NULL, NULL, NULL, NULL, NULL, NULL)");
    
    executeQuery("INSERT INTO `inquiry_status_history` (`id`, `inquiry_id`, `type`, `status`, `time`, `allocated_to`, `allocated_by`, `order_no`, `comments`, `duration`, `call_type`, `entered_by`, `call_status`, `medicine_status`, `patient_feedback`) VALUES (NULL, '$inquiy_id', 'Order Handling', '$status[$i]', '$currentDateTime', '$_SESSION[email]', '$seller', '$order_id[$i]', 'Edit $status[$i] Bill# $payment_receiving_id', NULL, NULL, NULL, NULL, NULL, NULL)"); 
