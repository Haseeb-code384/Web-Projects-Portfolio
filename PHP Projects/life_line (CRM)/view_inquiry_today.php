...
if ( isset( $_REQUEST[ 'order_status' ] ) ) {
  ...
  $sqlview = "SELECT * FROM `inquiry`  WHERE (date(created_at)=CURRENT_DATE or date(allocated_at)=CURRENT_DATE OR recall_date=CURRENT_DATE) AND  order_status='$order_status' $limit";
} else if ( isset( $_REQUEST[ 'call_status' ] ) ) {
  $call_status = "'" . $_REQUEST[ 'call_status' ] . "'";

  $sqlview = "SELECT * FROM `inquiry`  WHERE (date(created_at)=CURRENT_DATE or date(allocated_at)=CURRENT_DATE OR recall_date=CURRENT_DATE) AND  call_status=$call_status $limit";
} else {
  $call_status = 'call_status';

  $sqlview = "SELECT * FROM `inquiry`  WHERE (date(created_at)=CURRENT_DATE or date(allocated_at)=CURRENT_DATE OR recall_date=CURRENT_DATE) $limit";
}
...
<div class="content-wrapper">
  <div class="container-fluid">
    <?php breadcrumb(); ?>
    <div class="col-lg-12">
      <?php include("view_inquiry_all_today_tabs.php"); ?>
