...
if ( isset( $_REQUEST[ 'order_status' ] ) ) {
  ...
  $sqlview = "SELECT * FROM `inquiry`  WHERE record_type='Patient' AND allocated_to='$login_user' AND order_status='$order_status' $limit";
} else if ( isset( $_REQUEST[ 'call_status' ] ) ) {
  $call_status = "'" . $_REQUEST[ 'call_status' ] . "'";

  $sqlview = "SELECT * FROM `inquiry`  WHERE record_type='Patient' AND allocated_to='$login_user' AND call_status=$call_status $limit";
} else {
  $call_status = 'call_status';

  $sqlview = "SELECT * FROM `inquiry`  WHERE record_type='Patient' AND allocated_to='$login_user' $limit ";
}
...
<div class="content-wrapper">
  <div class="container-fluid">
    <?php breadcrumb(); ?>
    <div class="col-lg-12">
      <?php include("user_patient_all_tabs.php"); ?>
      <?php include("fix_header.php"); ?>
          ...
          <tbody>
            ...
            while ( $rowview = mysqli_fetch_array( $queryview ) ) {
              ?>
            <tr style="background-color: <?php echo showQuery("SELECT color FROM `status_list` WHERE status_name='$rowview[call_status]'") ?>;" style="background-color: <?php echo showQuery("SELECT color FROM `status_list` WHERE status_name='$rowview[call_status]'") ?>;"  onClick="chnage_row_clr(this,'lightgreen');" onDblClick="chnage_row_wclr(this,'white');"  >
              <td><input type="checkbox">
                <i onClick="window.open('inquiry_details.php?id=<?php echo $rowview[0]; ?>','height=100','width=100');"  title="More" class="fa fa-info-circle text-info"></i> <a target="new" href="update_inquiry_status.php?id=<?php echo $rowview[0]; ?>" title="Set Call Status"><i class="fa fa-check-square-o text-info"></i></a> <a href="update_inquiry_form.php?id=<?php echo $rowview[0]; ?>" target="new" title="Update"><i class="fa fa-edit text-success"></i></a></td>
