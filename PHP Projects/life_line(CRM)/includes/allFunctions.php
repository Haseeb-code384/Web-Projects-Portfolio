...
{
    $x=$email;
if ( $x == "Doctor Saima Chughtai" || $x == "Malik Nadeem" || $x == "admin" || $x == "Doctor Omar Chughtai" || $x == "Abdullah Arshad" ) 
{
    return true;
}
    else
...
function accumulated_bal($id,$end_date)
{
	...
	$d=$end_date;
		include('config.php');
$dr=showQuery("SELECT sum(amount) FROM `m_account_detail` WHERE m_accountid='$aid' AND info='Dr' AND tr_date<'$end_date'");
$cr=showQuery("SELECT sum(amount) FROM `m_account_detail` WHERE m_accountid='$aid' AND info='Cr' AND tr_date<'$end_date'");
$bal=$dr-$cr;
return $bal;
}
