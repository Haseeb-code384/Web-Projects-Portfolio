...
$id = $_REQUEST[ 'id' ];
$sql = "SELECT *,(SELECT patient_id FROM `inquiry` WHERE id=order_patient_id) AS 'patient_id' FROM `order_dispatch_info` WHERE order_id='$id'";
...
<table border="1" width="100%">
  ...
  <tr align="center">
    ...
    <td><?php echo $rowview['patient_id']; ?></td>
    <td><?php echo change_date_ddmmyyy($rowview[4]); ?></td>
...
<table border="1" width="100%">
  ...
  <tr align="center">
    ...
    <td><?php echo change_datetime_ddmmyyyhis($rowview[20]); ?></td>
    <td><?php echo change_datetime_ddmmyyyhis($rowview[21]); ?></td>
    <td><?php echo $rowview[22]; ?></td>
    <td><?php echo $rowview[23]; ?></td>
    <td><?php echo change_datetime_ddmmyyyhis($rowview[24]); ?></td>
    <td><?php echo change_datetime_ddmmyyyhis($rowview[25]); ?></td>
  </tr>
</table>
<table border="1" width="100%">
  <thead>
...
if ( check_admin( $_SESSION[ 'email' ] ) ) {
  ?>
<center>
  <i title="Status History" onClick="window.open('order_status_history.php?id=<?php echo $rowview[0]; ?>','height=200','width=200');" class="fa fa-history text-primary">
  <button>Status History</button>
  </i> <a target="new" href="update_order_status.php?id=<?php echo $rowview[0]; ?>" title="Set Order Status"><i class="fa fa-reorder text-warning">
  <button>Set Order Status</button>
  </i></a> <a target="new" href="update_order_checklist.php?id=<?php echo $rowview[0]; ?>" title="Check List"><i class="fa fa-check-square-o text-info"></i>
  <button>Check List</button>
  </a> <a target="new" href="update_order_form.php?id=<?php echo $rowview[0]; ?>" title="Update"><i class="fa fa-edit text-success">
  <button>Update</button>
  </i></a> <a target="new" href="flowchart.php?order_id=<?php echo $rowview[0]; ?>" title="Update"><i class="fa fa-edit text-success">
  <button>Order Stages</button>
  </i></a>
</center>
<?php } ?>
</body>
