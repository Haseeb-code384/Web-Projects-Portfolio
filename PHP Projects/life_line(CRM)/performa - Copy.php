...
<!--					EXEC SIDE (CRE-STAFF) ABSENT-->
						<td><?php 
						echo showQuery("select (SELECT count(*) FROM employee,employee_shift_allocation e LEFT JOIN attendence ON e.employee = attendence.emp_no AND e.status='Allocated' WHERE employee.id=e.employee AND  attendence.emp_no IS NULL AND status='Allocated' AND e.date between '$date_start' AND '$date_start' AND employee.employee_type='$employee_type' AND employee.branch IN (SELECT id FROM account_subhead WHERE head_name='$head_name' )) - (SELECT count(*) FROM employee_shift_allocation,employee WHERE employee_shift_allocation.employee=employee.id AND employee_shift_allocation.date='$date_start' AND employee.branch IN (SELECT id FROM account_subhead WHERE head_name='$head_name') AND (employee.employee_type='$employee_type') AND status IN (SELECT status_name FROM `status_list`))");
						?></td>
...
<!--					ADM  SIDE (CRE-STAFF) PRESENT-->
		...
		<div>
			...
			$sqlview="SELECT employee.army_no,employee.rank,employee.name,employee.branch,e.date,e.status FROM employee,employee_shift_allocation e LEFT JOIN attendence ON e.employee = attendence.emp_no AND e.status IN (SELECT status_name FROM `status_list`) WHERE employee.id=e.employee AND attendence.emp_no IS NULL AND status IN (SELECT status_name FROM `status_list`) AND e.date between '$date_start' AND '$date_start' AND (employee.employee_type='C/STAFF' OR employee.employee_type='CRE-STAFF') AND employee.branch IN (SELECT id FROM account_subhead WHERE head_name='EXEC SIDE' )";
				//echo $sqlview;
				$queryview=mysqli_query($con,$sqlview);
				while($rowview=mysqli_fetch_array($queryview))
				{
				...
				<tr align="center">
					...
					<td align="left"><?php echo $rowview[0]; ?></td>
					<td><?php echo $rowview[1]; ?></td>
					<td align="left"><?php echo $rowview[2]; ?></td><td><?php echo  showQuery("SELECT subhead_name FROM `account_subhead` WHERE id='$rowview[3]'") ?></td>
				...
				<?php
				$i=1;
				//$sqlview="SELECT employee.army_no,employee.rank,employee.name,employee.branch,e.date  FROM employee,employee_shift_allocation e LEFT JOIN attendence ON e.employee = attendence.emp_no AND e.status='Allocated' WHERE employee.id=e.employee AND attendence.emp_no IS NULL AND status='Allocated' AND e.date between '$date_start' AND '$date_start' AND employee.employee_type='C/STAFF' AND employee.branch IN (SELECT id FROM account_subhead WHERE head_name='ADM SIDE' )";
				$sqlview="SELECT employee.army_no,employee.rank,employee.name,employee.branch,e.date,e.status FROM employee,employee_shift_allocation e LEFT JOIN attendence ON e.employee = attendence.emp_no AND e.status IN (SELECT status_name FROM `status_list`) WHERE employee.id=e.employee AND attendence.emp_no IS NULL AND status IN (SELECT status_name FROM `status_list`) AND e.date between '$date_start' AND '$date_start' AND (employee.employee_type='C/STAFF' OR employee.employee_type='CRE-STAFF') AND employee.branch IN (SELECT id FROM account_subhead WHERE head_name='ADM SIDE' )";
				//echo $sqlview;
				$queryview=mysqli_query($con,$sqlview);
				while($rowview=mysqli_fetch_array($queryview))
				{
