...
if ( isset( $_REQUEST[ 'submit' ] ) ) {
  ...
  executeQuery( "INSERT INTO `order_payment_cart`(`id`, `order_id`, `tracking_id`, `user`, `time`) (SELECT null,order_id,tracking_id,'$user','$currentDateTime' FROM order_dispatch_info WHERE tracking_id='$tracking_id')" );
}
?>
<!DOCTYPE html>
<html>
...
<body onLoad="calculate_grand_total('.order_amount','total_order_amount');calculate_grand_total('.courier_cost','total_courier_cost');calculate_grand_total('.package_weight','total_package_weight');">
<div class="content-wrapper" >
  ...
  <form method="post" onSubmit="return confirm('Are You Sure To Edit This Data? This Change Will Be Permanent');">
    ...
    <?php include("fix_header.php"); ?>
        ...
        <tbody>
          <?php
          //$sql_view = "SELECT * FROM order_dispatch_info,order_payment_cart WHERE order_dispatch_info.order_id=order_payment_cart.order_id AND order_payment_cart.user='$user'  ORDER BY `order_payment_cart`.`time`  ASC";
          $sql_view = "SELECT * FROM order_dispatch_info WHERE order_dispatch_info.payment_receiving_id='$id';";
          //                    echo $sql_view;
        ...
        <?php
        $sql_view = "SELECT * FROM order_dispatch_info,order_payment_cart WHERE order_dispatch_info.order_id=order_payment_cart.order_id AND order_payment_cart.user='$user'  ORDER BY `order_payment_cart`.`time`  ASC";
        //          $sql_view = "SELECT * FROM order_dispatch_info WHERE order_dispatch_info.payment_receiving_id='1';";
        //                    echo $sql_view;
        $query_view = mysqli_query( $con, $sql_view );
        //          $i = 0;
        while ( $row_view = mysqli_fetch_array( $query_view ) ) {
        ...
        <tr class="h6">
          <th colspan="7" ><?php echo $i; ?> Parcels</th>
...
-->
      ...
      <div class="col-sm-2">
        <label><strong>Cheque Amount</strong></label>
        <input type="number" name="cheque_amount" class="form-control" required value="<?php echo $row_deposit['cheque_amount'] ?>">
      </div>
      <div class="col-sm-2">
