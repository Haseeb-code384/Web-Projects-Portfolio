...
{ 
    echo 'bg-danger text-white'; 
       ...
       <input type="text" name="patient_id"  maxlength="6"  class="form-control"  value="<?php echo $row_data['patient_id'] ?>" >
        </div>
            <div class="col-sm-6">
   ...
   <?php setSelectOptionIfNotNull($row_data['marital_status']) ?>
        ...
        <div class="col-sm-12">
          ...
          <select multiple name="disease[]" id="disease" style="height: 300px;" title="Press CTRL To Select Multiple"  class="form-select">
            ...
            <?php populateDDsel("disease_category WHERE disease NOT IN (SELECT disease FROM `inquiry_disease` WHERE inquiry_id='$id')","disease_category","disease_category",""); ?>
          </select>
          </div>
      </div>
		       <script>
...
<label class="btn-sm btn-outline-danger col-12" style="<?php
    ...
    <div class="col-12 text-center"> <br>
        ...
        <option  value="<?php echo $row_data['permanent_allocation'] ?>"><?php echo showbool($row_data['permanent_allocation']) ?></option>
